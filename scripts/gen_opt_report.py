...
def main() -> int:
    ...
    A('</tbody></table>')
    A('<div class="note"><b>这张表里「系统优化方案.html」那一行，是<b>诊断时点</b>的本页：</b>'
      '诊断快照在优化动作<b>开始之前</b>拍摄，此后<b>冻结</b>（归档等后续动作不再刷新它）—— '
      '而本页在这之后又改了几轮。这与索引页「不把自身列进自己的表」是同一个坑：'
      '文档自身的体量在它被生成的那一刻<b>还没确定</b>。'
      '所以该行数字应当读作“诊断时点的值”，而不是你现在看到的这一版；'
      '本轮把这个矛盾写在表下，而不是等读者自己去发现。'
      '（顺带一条口径：快照冻结是<b>刻意</b>的 —— 后续动作会改变磁盘，'
      '若重算快照，“根目录 37 个残留”会变成 0，报告就会与自己的执行记录打架。）</div>')
    ...
    A('<div class="card"><h3><span class="pri p2">P2-1</span> 给历史无锚点产物补锚点或降级标注（已执行）</h3>')
    ...
    A('<p><b>结果</b>：索引页产物表已有<b>可校验性</b>列（每行非空），并补了一段'
      '<b>覆盖对账</b>：行数 = 可机器校验 + 历史产物，且必须等于“根目录 HTML 数 − 本页自身”。'
      '三个数由生成器<b>现场清点并断言</b>（不满足就不出索引页），不手写。'
      '具体数字见 <code>交付物索引.html</code> §1（刻意不复述在此 —— '
      '它会随产物增减而变，写进本页就立刻过期）。</p>')
    A('<p><b>验收判据</b>：索引页上每一份产物的“可校验性”状态都非空，'
      '且“已校验 / 未校验”的计数与锚点统计一致。<b>已达成</b>。</p>')
    A('<p><b>风险与回滚</b>：低。不产生错误结论；回滚 = 去掉该列与对账段。'
      '⚠️ 一处边界：索引页自身也在“无锚点”名单里，而它<b>不列自身</b>'
      '（递归无解）—— 对账时要把这一个减掉，否则永远差 1 条。本项目已减。</p></div>')
    ...
    A('<p><b>怎么做</b>：先在原型场做对照实验，<b>增益必须超过 1 个标准误</b>才考虑落底座；'
      '落底座走 C2 列的完整闸门。</p>')
    A('<p><b>本轮做到哪一步（不是“待办”，而是已交付一份任务书）</b>：'
      '实证过这类项<b>不是一轮能完成</b>的 —— 改底座影响所有下游，'
      '且“成功”的判据是<b>增益超标准误</b>，必须跑多臂×多种子×多数据集的对照。'
      '硬塞进本轮只会得到“改完了、没验证”，而那正是 R1 “7 次接受中 2 次有害”的成因。'
      '已交付：<code>证据/08_底座能力缺口现状与实验骨架.md</code>'
      '（三项缺口的<b>现读核实</b> + 三臂对照骨架 + 4 条否决项 + 落底座门禁）。</p>')
    A('<p><b>本轮的关键发现</b>：三项缺口的现状核实必须<b>现读代码</b>而不是引旧记录，'
      '因为存在一类最骗人的情形 —— <b>关键词只在注释里命中</b>（代码写着“为什么不用 X”），'
      '看起来像“有实现”。本轮为此给探针写了三态判定与自检，'
      '而自检<b>第一次跑就抓出了判据自身的 bug</b>（详见证据 08 §四）。</p>')
    A('<p><b>验收判据</b>：每项能力都有独立对照与显著性判据；'
      '无显著增益的一律<b>不落</b>并写明理由。<b>未达成 —— 待单独实验轮次</b>。</p>')
    A('<p><b>风险与回滚</b>：高（改底座 + 影响所有下游结果）。'
      '这正是本轮把它留在 P2 而不是 P0 的原因。</p></div>')

    A('<div class="card"><h3><span class="pri p2">P2-3</span> 记忆写入纪律与技能组织（① 已执行 / ② 清单已出）</h3>')
    ...
    A('<p><b>结果</b>：① <b>已执行</b> —— <code>MEMORY.md</code> 顶部已含写入纪律段'
      '（含“本文件有多个写入者，一次性压缩治不了并发追加”）。'
      '② <b>清单已出</b>：<code>证据/09_同前缀技能族清单.md</code>，'
      '含逐族触发词重叠度（Jaccard）与“本质区别”两栏 —— 它把“名字相似”'
      '与“职责重叠”分开看，<b>不替人下结论</b>。</p>')
    A(f'<p><b>验收判据</b>：<code>MEMORY.md</code> ≤ {S("d.mem_limit")} 字符；'
      f'顶部含写入纪律说明；技能目录文件数与条目数<b>不变</b>。'
      f'<b>①②均已达成</b>；裁决本身属人工判断，不在本方案范围内。</p>')
    A('<p><b>风险与回滚</b>：低。⚠️ 且注意：只靠写纪律<b>不能根治</b>并发追加，'
      '若多会话同时写，仍会超限 —— 那是机制问题，需要“单一写入者”约定，'
      '本轮只能做到“超限时可见”。</p></div>')

    # ══════════════════════════════════════════════ 6 已执行 / 待排期
    A('<h2>6. 本轮已执行 vs 待排期</h2>')
    A('<p>边界一句话说清：<b>“已执行”= 有判据、有证据、可复算；'
      '“待单独实验轮次”= 需要跑对照实验才能下结论</b> —— 两者不能混为一谈，'
      '所以本表刻意不写“全部完成”。</p>')
    ...
    A('<tr><td>P2-1 无锚点产物降级标注</td><td class="ctr"><span class="tag ok">已执行</span></td>'
      '<td><code>交付物索引.html</code>（可校验性列 + 覆盖对账）</td></tr>')
    A('<tr><td>P2-3① 记忆写入纪律</td><td class="ctr"><span class="tag ok">已执行</span></td>'
      '<td><code>.workbuddy/memory/MEMORY.md</code> 顶部</td></tr>')
    A('<tr><td>P2-3② 同前缀技能族清单</td><td class="ctr"><span class="tag ok">已出清单</span></td>'
      '<td><code>证据/09_同前缀技能族清单.md</code>（<b>裁决待你</b>）</td></tr>')
    A('<tr><td>P2-2 底座能力缺口：现状核实 + 实验骨架</td>'
      '<td class="ctr"><span class="tag ok">已交付任务书</span></td>'
      '<td><code>证据/08_底座能力缺口现状与实验骨架.md</code></td></tr>')
    A('<tr><td>P2-2 <b>实际改底座</b></td><td class="ctr"><span class="tag no">待单独实验轮次</span></td>'
      '<td>需对照实验 + 增益 >1 SE；<b>本方案书不把它当成可一轮做完的事</b></td></tr>')
    A('</tbody></table>')
