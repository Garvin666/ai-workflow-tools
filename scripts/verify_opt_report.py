...
from _opt_data import collect, render, WORKSPACE, RECON       # noqa: E402
...
def self_test(report: Path) -> int:
    """三态阴性对照：① 基线必须 0；② 脚本一字不改、只改一位数字 → 必须 1；③ 文件缺失 → 必须 2。"""
    print("=" * 88)
    ...
    ok = (code0 == 0 and code1 == 1 and code2 == 2 and code3 == 2 and code4 == 2)
    print()
    print(f"阴性对照结论: {'五态齐备且判据有效 ✔' if ok else '✘ 判据有问题，不能声称方案书已验'}")
    print(f"  （基线 {code0} / 扰动 {code1} / 缺失 {code2} / 破坏1处 {code3} / 全破坏 {code4}；"
          f"期望 0/1/2/2/2）")
    return 0 if ok else 1
